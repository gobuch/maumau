...
// CardGame defines all available cards inside the game.
// This function is called to generate a stack for the game.
// Except for the wish cards, every card needs a value and a color.
...
// Card defines the properties of a card.
type Card struct {
	ID          string `json:"id"`
	Color       string `json:"color"`
	Value       string `json:"value"` // number or name e.g. 1, K, J
	Label       string `json:"label"` // the label can differ from the value
...
// Check validates the next card due to the rules of the game.
// In maumau the next card need the same color or value. Wish cards
...
// peek shows the last card but does not affect
// the stack
...
// len returns the number of the cards in the stack
func (cs *CardStack) len() int {
	return len(cs.Cards)
}

// shuffle the cards inside a stack
